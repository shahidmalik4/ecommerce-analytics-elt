...
def clean_and_transform(df_orders: pd.DataFrame, df_customers: pd.DataFrame) -> pd.DataFrame:
    ...
    # 1. Merge DataFrames (Left join on orders to customers)
    df_merged = pd.merge(
        df_orders, 
        df_customers, 
    ...
    # Deduplicate based on 'order_id' (keep the first occurrence)
    ...
    # Impute missing quantities with 1
    df_merged['quantity'].fillna(1, inplace=True)
    logger.info("Imputed missing quantities with 1.")
    
    # Impute missing region values with 'Unknown'
    df_merged['region'].fillna('Unknown', inplace=True)
    logger.info("Imputed missing regions with 'Unknown'.")

    # Fix bad foreign keys (orders without a matching customer)
    # The merge results in NaN for customer_name/region for bad FKs. 
    # The region is already handled, now handle customer_name.
    ...
    # Convert negative prices to absolute value for 'price_usd'
    df_merged['price_usd'] = df_merged['raw_price'].abs()

    # Calculate derived metric (total_sale)
    df_merged['total_sale'] = df_merged['price_usd'] * df_merged['quantity']
    
    # 3. Add Metadata Column (THE FIX)
    # Ensure creation_timestamp is present for Pydantic validation and final column selection.
    df_merged['creation_timestamp'] = datetime.now()
    logger.info("Added 'creation_timestamp' metadata column.")
    
    # 4. Final Column Selection and Pydantic Validation
    # Select the columns matching the CleanedOrder schema definition
    df_clean = df_merged[[
